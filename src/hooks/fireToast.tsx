...
const customJson = dataJSON as JsonData;
  ...
  if (alertSettings) {
    for (const alertSetting of JSON.parse(alertSettings)) {
      ...
      if (alertSetting.id == 'ALL') {
        Object.keys(customJson).map((id: string) => {
          const condition =
            alertSetting.criterion == '0'
              ? value <= -1 * (customJson[id][para] as number)
              : alertSetting.criterion == '1' || alertSetting.criterion == '3'
                ? value >= customJson[id][para]
                : alertSetting.criterion == '2'
                  ? value <= customJson[id][para]
                  : value == customJson[id][para];
          const realValue =
            alertSetting.criterion == '0'
              ? (customJson[id][para] as number) * -1
              : customJson[id][para];
          if (condition) {
